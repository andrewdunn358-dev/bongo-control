...
// Deliberately does NOT list every icon. Precaching is all-or-nothing:
// one 404 rejects addAll() and the whole install fails, taking offline
// support with it. This list has already been wrong twice for exactly
// that reason - it named /apple-touch-icon.svg and then /icon.svg,
// both of which were later deleted during the icon rework. Only the
// shell files genuinely needed to boot offline are listed now, and
// none of them are icons; the icons are same-origin and get cached on
// demand by the fetch handler below.
