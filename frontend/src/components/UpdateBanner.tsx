...
export function UpdateBanner() {
  ...
  // Only reload when WE initiated activation (the user clicked Reload).
  // Without this, the first-ever load's clients.claim() fires
  // controllerchange and would reload the page seconds after it opened.
  ...
  useEffect(() => {
    ...
    const onControllerChange = () => {
      if (userInitiated.current) window.location.reload();
    };
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 rounded-2xl px-4 py-3 bg-aurora-teal/15 ring-1 ring-inset ring-aurora-teal/40 backdrop-blur-md">
        ...
        <button
          type="button"
          onClick={() => { userInitiated.current = true; waiting.postMessage({ type: 'SKIP_WAITING' }); }}
