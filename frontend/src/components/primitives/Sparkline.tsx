import { useMemo } from 'react';

interface Props {
  data: number[];
  width?: number;
  height?: number;
  stroke?: string;
  fill?: string;
  strokeWidth?: number;
  padding?: number;
  className?: string;
  /**
   * Smallest y-range the chart will scale to. Without this, the axis
   * fits exactly min..max, so a reading that barely moves gets its
   * noise amplified to full height - a resting battery flipping
   * between 12.88 and 12.89 V (just 2dp rounding) rendered as a
   * dramatic square wave, implying voltage swings that never happened.
   *
   * Set it to roughly the smallest change actually worth seeing for
   * that measurement: about 0.4 V for a 12 V bank, tens of watts for
   * solar, a degree or two for temperature.
   */
  minRange?: number;
}

/** Small inline SVG sparkline. Drops non-finite points; never fabricates.
 *
 * Uses viewBox + CSS width:100% rather than fixed width/height SVG
 * attributes, so it scales down to fit a narrower container instead of
 * getting silently clipped by the card's overflow-hidden. Reported
 * symptom this fixes: Power.tsx's merged Battery/Solar cards (which
 * copied width={520} verbatim from the original standalone pages) cut
 * the sparkline off on a phone screen - the card was hiding whatever
 * didn't fit, not shrinking to show it. width/height props still set
 * the internal coordinate system and aspect ratio (and cap how large it
 * gets on a wide screen via maxWidth) - existing callers (Home.tsx's
 * 260px sparklines, which already fit comfortably on mobile) render
 * identically; this only changes behaviour for a container actually
 * narrower than the given width. */
export function Sparkline({
  data,
  width = 220,
  height = 54,
  stroke = '#22d3ee',
  fill = 'rgba(34,211,238,0.25)',
  strokeWidth = 2,
  padding = 2,
  className,
  minRange = 0,
}: Props) {
  const paths = useMemo(() => {
    const values = data.filter((v) => Number.isFinite(v));
    if (values.length < 2) return { line: '', area: '' };
    let lo = Math.min(...values);
    let hi = Math.max(...values);
    // Widen a too-narrow range around its midpoint, so genuinely flat
    // data draws as a flat line rather than amplified noise.
    if (hi - lo < minRange) {
      const mid = (lo + hi) / 2;
      lo = mid - minRange / 2;
      hi = mid + minRange / 2;
    }
    const range = hi - lo || 1;
    const stepX = (width - padding * 2) / Math.max(values.length - 1, 1);
    const points = values.map((v, i) => {
      const x = padding + i * stepX;
      const y = padding + (1 - (v - lo) / range) * (height - padding * 2);
      return [x, y] as const;
    });
    const line = points.map(([x, y], i) => `${i === 0 ? 'M' : 'L'}${x.toFixed(1)},${y.toFixed(1)}`).join(' ');
    const [firstX] = points[0];
    const [lastX] = points[points.length - 1];
    const area = `${line} L${lastX.toFixed(1)},${height - padding} L${firstX.toFixed(1)},${height - padding} Z`;
    return { line, area };
  }, [data, width, height, padding, minRange]);

  const gradId = `spark-${stroke.replace('#', '')}`;

  if (!paths.line) {
    return (
      <svg viewBox={`0 0 ${width} ${height}`} className={className} style={{ width: '100%', height: 'auto', maxWidth: width }}>
        <text x={width / 2} y={height / 2 + 4} textAnchor="middle" fill="rgba(148,163,184,0.5)" fontSize="10">
          no history yet
        </text>
      </svg>
    );
  }

  return (
    <svg viewBox={`0 0 ${width} ${height}`} className={className} style={{ width: '100%', height: 'auto', maxWidth: width }}>
      <defs>
        <linearGradient id={gradId} x1="0" x2="0" y1="0" y2="1">
          <stop offset="0%" stopColor={fill} />
          <stop offset="100%" stopColor="transparent" />
        </linearGradient>
      </defs>
      <path d={paths.area} fill={`url(#${gradId})`} />
      <path
        d={paths.line}
        stroke={stroke}
        strokeWidth={strokeWidth}
        strokeLinecap="round"
        strokeLinejoin="round"
        fill="none"
      />
    </svg>
  );
}
