...
const accentClass: Record<CardAccent, string> = {
  ...
  neutral: "from-white/[0.06] via-white/[0.025] to-transparent border-ink/[0.08]",
};
      ...
      <div className={`relative ${compact ? "p-5" : "p-5 sm:p-6 xl:p-7"}`}>
        {label && (
          <div className="mb-5 flex items-center gap-2 text-text-secondary">
            {icon && <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-ink/[0.06] text-battery ring-1 ring-white/[0.06]">{icon}</span>}
