    ...
    <GlassCard className="col-span-12 lg:col-span-6" data-testid={HIST.chart(domainKey)}>
      ...
      <div className="h-[280px]">
        {isLoading ? (
          ...
          // Distinct from "no samples": on a project whose whole ethos is
          // "gaps are honest", claiming no data when we simply couldn't
          // fetch would be a lie.
          <div className="h-full grid place-items-center text-sm text-status-amber text-center px-4">couldn&apos;t load history — check the connection and try again</div>
        ) : series.length === 0 ? (
