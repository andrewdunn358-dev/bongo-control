...
export function Roof() {
  ...
  // Safety net: if the page is hidden or the window loses focus, stop
  // sending holds. The backend watchdog would catch it anyway - this
  // just makes it immediate. Deliberately its own effect, separate from
  // the interval cleanup below: this one depends on `active` (so onLeave
  // always closes over the current value), which means it re-runs every
  // time a hold starts or stops. If clearInterval(timerRef.current) lived
  // in THIS effect's cleanup, it would fire the instant `active` flips to
  // a direction - i.e. right after start() creates the interval - killing
  // it before it ever got to run a second tick. That was the actual bug:
  // every hold sent exactly one request, then silence, and ~1.5s later
  // the backend's watchdog stopped the roof thinking the app had gone
  // away, no matter how long the button was actually held.
  useEffect(() => {
  ...
  // Interval cleanup on unmount only - empty deps so this never re-runs
  // (and never clears a freshly-created interval) just because `active`
  // changed.
  useEffect(() => {
    return () => {
