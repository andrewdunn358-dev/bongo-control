...
import { Cloud, CloudRain, CloudSnow, Sun, CloudSun, CloudFog, Sunset, Sunrise, Droplets, Gauge } from 'lucide-react';
...
/** One column in the 7-day strip. Now a button - tap a day to expand
 *  its detail panel below (precipitation chance, solar radiation,
 *  sunrise/sunset - data already fetched and typed, just not shown
 *  anywhere before). */
function DayCol({ date, day, i, selected, onSelect }: { date?: string | null; day: DailyWeather | undefined; i: number; selected: boolean; onSelect: () => void }) {
  ...
  return (
    <button
      ...
      disabled={!day}
      className={
        'flex-1 min-w-0 text-center rounded-xl py-3 px-1 transition-colors ' +
        (selected
          ? 'bg-aurora-teal/10 ring-2 ring-aurora-teal/60'
          : 'bg-ink/[0.03] ring-1 ring-inset ring-ink/10 hover:bg-ink/[0.06] disabled:hover:bg-ink/[0.03] disabled:cursor-default')
      }
    >
...
/** Expanded detail for whichever day is currently selected in the
...
function DayDetail({ date, day, i }: { date?: string | null; day: DailyWeather; i: number }) {
  ...
  const Icon = meta.Icon;
  return (
    <div className="mt-3 rounded-2xl bg-ink/[0.03] ring-1 ring-inset ring-ink/10 p-4">
      <div className="flex items-center gap-3">
        <div className="h-11 w-11 shrink-0 rounded-2xl grid place-items-center bg-ink/[0.04] ring-1 ring-inset ring-ink/10">
          <Icon size={22} color={meta.color} />
        </div>
        <div>
          <div className="text-sm font-medium">{weekday(date ?? day.date, i)}{day.date ? ` · ${day.date}` : ''}</div>
          <div className="text-xs text-ink-muted">{day.weather_description || wmoLabel(day.weather_code)}</div>
        </div>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 text-xs">
        <div>
          <div className="text-ink-faint flex items-center gap-1"><Droplets size={11} /> Rain chance</div>
          <div className="num text-sm mt-0.5">{day.precipitation_probability_max_pct == null ? DASH : fmtPct(day.precipitation_probability_max_pct)}</div>
        </div>
        <div>
          <div className="text-ink-faint flex items-center gap-1"><Gauge size={11} /> Solar energy</div>
          <div className="num text-sm mt-0.5">{day.shortwave_radiation_sum_mj == null ? DASH : fmtMJ(day.shortwave_radiation_sum_mj)}</div>
        </div>
        <div>
          <div className="text-ink-faint flex items-center gap-1"><Sunrise size={11} /> Sunrise</div>
          <div className="num text-sm mt-0.5">{day.sunrise ? fmtLocalTime(day.sunrise) : DASH}</div>
        </div>
        <div>
          <div className="text-ink-faint flex items-center gap-1"><Sunset size={11} /> Sunset</div>
      ...
      <div className="grid grid-cols-12 gap-4 lg:gap-6">
        ...
        <GlassCard glow="teal" className="col-span-12 lg:col-span-6 p-6" data-testid={WEATHER.currentTemp}>
          ...
          {selectedIdx != null && days[selectedIdx] && (
            <DayDetail date={days[selectedIdx]?.date} day={days[selectedIdx] as DailyWeather} i={selectedIdx} />
          )}
        </GlassCard>
