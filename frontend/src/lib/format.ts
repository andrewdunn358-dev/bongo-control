...
const DISTANCE_UNIT_KEY = 'vanos.distanceUnit';

/** Miles by default — this is a UK van, and every distance in the app
 *  (trip totals, "how far is this fuel station") should read the way a
 *  UK driver actually thinks about distance. 'km' only if explicitly
 *  chosen in Settings. */
export function getDistanceUnit(): 'mi' | 'km' {
  try {
    return localStorage.getItem(DISTANCE_UNIT_KEY) === 'km' ? 'km' : 'mi';
  } catch {
    return 'mi';
  }
}
export function setDistanceUnit(unit: 'mi' | 'km') {
  try { localStorage.setItem(DISTANCE_UNIT_KEY, unit); } catch { /* ignore */ }
}
