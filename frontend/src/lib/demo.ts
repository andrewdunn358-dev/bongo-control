...
export async function demoRequest<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  // Voice control — no real mic/speaker in the public demo, so this
  // simulates a working, already-used setup rather than an unconfigured one.
  if (p === '/voice-control/status') {
    return R({
      enabled: true,
      configured: true,
      listening: true,
      ...
      wake_word: 'computer',
      last_wake_at: Date.now() / 1000 - 340,
      last_command_text: 'turn the lights on',
      last_reply_text: 'Turning the lights on.',
      last_error: null,
      voice_controllable_relays: ['heater', 'lights', 'radio', 'tv'],
    });
  }
  if (p === '/voice-control/test') {
    return R({
      enabled: true,
      configured: true,
      listening: true,
      ...
      playback_device: 'hw:1,0',
      wake_word: 'computer',
      last_wake_at: Date.now() / 1000,
      last_command_text: '(demo mode — nothing was actually recorded)',
      last_reply_text: "That's a canned demo reply — try this for real once it's deployed.",
      last_error: null,
      voice_controllable_relays: ['heater', 'lights', 'radio', 'tv'],
    });
  }
  if (p === '/voice-control/speak-test') {
    return R({
      enabled: true,
      configured: true,
      listening: true,
      ...
      wake_word: 'computer',
      last_wake_at: Date.now() / 1000,
      last_command_text: '(demo mode — no real speaker to play through)',
      last_reply_text: "That's a canned demo reply — try this for real once it's deployed.",
      last_error: null,
      voice_controllable_relays: ['heater', 'lights', 'radio', 'tv'],
    });
  ...
  if (p === '/ai/status') return R({ configured: true, persona_name: 'Ron' });
  if (p === '/ai/chat') {
  ...
  if (p === '/internet-radio/play' || p === '/internet-radio/pause' || p === '/internet-radio/resume')
    return R({ available: true, running: true, playing: p !== '/internet-radio/pause', stream_url: 'https://radio.3bty.co.uk/listen/btybar/radio.mp3', configured_stream_url: 'https://radio.3bty.co.uk/listen/btybar/radio.mp3' });
  ...
  if (p.startsWith('/radio-directory/click/')) return R({ ok: true });
  if (p === '/wifi/scan')
    return R({
      networks: [
