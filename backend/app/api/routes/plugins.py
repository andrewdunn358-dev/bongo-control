...
# Gated: plugin config holds secrets (e.g. the Victron BLE encryption_key)
# and enable/disable can swap real telemetry for simulated data.
router = APIRouter(prefix="/api/plugins", tags=["plugins"], dependencies=[Depends(require_app_token)])

# Secret-bearing plugin config keys — never echoed back in a GET. A
# "<key>_set" boolean is exposed instead so the UI can show "configured"
# without ever handling the value (same rule as config.py's SECRET_KEYS).
PLUGIN_SECRET_KEYS = {"encryption_key", "password", "token", "api_key"}


def _redact_plugin_config(data: dict) -> dict:
    out = dict(data)
    for key in list(out.keys()):
        if key in PLUGIN_SECRET_KEYS:
...
async def get_plugin_config(name: str) -> dict:
    if _get_manager().get(name) is None:
        raise HTTPException(status_code=404, detail=f"Unknown plugin '{name}'")
    return _redact_plugin_config(configuration_service.get_plugin_config(name))
...
async def update_plugin_config(name: str, body: PluginConfigUpdate) -> dict:
    ...
    if _get_manager().get(name) is None:
        raise HTTPException(status_code=404, detail=f"Unknown plugin '{name}'")
    # An empty secret means "leave unchanged" — so re-saving the form
    # without re-typing the encryption_key keeps the stored one (the GET
    # blanks it), rather than wiping it.
    incoming = {k: v for k, v in body.config.items() if not (k in PLUGIN_SECRET_KEYS and not str(v or "").strip())}
    return _redact_plugin_config(configuration_service.update_plugin_config(name, incoming))
