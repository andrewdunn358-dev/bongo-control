...
class RenogyMPPTPlugin(Plugin):
    ...
    def __init__(self, bus: TelemetryBus) -> None:
        ...
        self._peak_watts_date = date.today()
    ...
    async def _publish(self, data: dict[str, Any]) -> None:
        ...
        # Reset the running peak at the day rollover, otherwise
        # "peak today" silently becomes "peak since boot" (matches the
        # Victron plugin's per-date reset).
        today = date.today()
