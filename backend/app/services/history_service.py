...
class HistoryService:
    ...
    def _prune(self) -> None:
        ...
        # lazily to avoid an import cycle at module load. location_history
        # is deliberately NOT pruned here — it's the Trips travel journal,
        # kept long-term, and already rate-limited at write time (50m/10min
        # threshold) so its growth is slow and intentional.
        try:
            ...
            poi_service.prune_cache()
            ai_recommendations_service.prune_cache()
            # Ofcom's terms cap how long their coverage data may be
            # retained, so this prune is a licence obligation, not just
            # SD-card hygiene. See coverage_service.CACHE_TTL_SECONDS.
            coverage_service.prune_cache()
        except Exception as e:  # noqa: BLE001 - maintenance must never crash the loop
